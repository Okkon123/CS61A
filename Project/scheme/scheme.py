...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operator_first=scheme_eval(first,env)
        validate_procedure(operator_first)
        rest=rest.map(lambda x:scheme_eval(x,env))
        return scheme_apply(operator_first,rest,env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(func_name,do_lambda_form(expr,env))
        return func_name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions==nil:
        ...
        if is_false_primitive(first_val):
            return False
        elif expressions.rest==nil and is_true_primitive(first_val):
            return first_val
        else:
...
def do_or_form(expressions, env):
    ...
    if expressions==nil:
        ...
        if is_true_primitive(first_val):
            return first_val
        elif is_true_primitive(first_val) and expressions.rest==nil:
            return first_val
        else:
...
def make_let_frame(bindings, env):
    ...
    while bindings!=nil:
        ...
        names=Pair(para,names)
        values=Pair(scheme_eval(vals.first,env),values)
        clause=clause.rest
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN PROBLEM 19
        "*** YOUR CODE HERE ***"
        return original_scheme_eval(result.expr,result.env)
        # END PROBLEM 19
