...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        tmp=self.place
        i,t=0,0
        while i<self.min_range and tmp!=beehive:
            i,t=i+1,t+1
            tmp=tmp.entrance
        while len(tmp.bees)==0 and tmp!=beehive:
            t+=1
            tmp=tmp.entrance
        if len(tmp.bees)!=0 and tmp!=beehive:
            if self.min_range<=t<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # else:
        #     for x in tmp:
        #         Insect.reduce_armor(x,amount+self.damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting:
            ...
            if rANTdom_else_none(self.place.bees):
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.is_impostor:
            ...
            tmp=self.place.exit
            while tmp:
                if tmp.ant and tmp.ant.buff==False:
                    tmp.ant.damage=tmp.ant.damage*2
                    tmp.ant.buff=True
                tmp=tmp.exit
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        tmp=2-self.insects_shot*0.05-distance*0.2
        if tmp>0:
            return tmp
        else:
            return 0 
