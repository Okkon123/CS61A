...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for x in player.deck.cards:
            x.attack=x.attack+other_card.attack
            x.defense=x.defense+other_card.defense
        lst=[]
        for y in range(len(opponent.deck.cards)):
            if opponent.deck.cards[y].attack!=other_card.attack or opponent.deck.cards[y].defense!=other_card.defense:
                lst=lst+opponent.deck.cards[y]
