...
def subseqs(s):
    ...
    if s==[]:
        ...
        for i in range(len(s)):
            lst=lst+[x for x in subseqs(s[:i]+s[i+1:]) if x not in lst]
    return lst
...
def make_advanced_counter_maker():
    ...
    1
    """
    dic_gobal={'global-count':0}
    def make_counter():
        nonlocal dic_gobal
        dic_new={'count':0}
        def helper(x):
            nonlocal dic_gobal,dic_new
            "*** YOUR CODE HERE ***"
...
def shuffle(cards):
    ...
    shuffled = []
    for i in range(half):
        shuffled=shuffled+[cards[i]]+[cards[half+i]]
    return shuffled


def insert1(link, value, index):
    ...
    if index==0:
        ...
        insert1(link.rest,value,index-1)
