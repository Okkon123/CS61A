...
def prune_min(t):
    ...
    if not t.is_leaf():
        ...
        if branch[0].label<=branch[1].label:
            t.branches.remove(branch[1])
        else:
            t.branches.remove(branch[0])
        for b in t.branches:
            prune_min(b)
...
def num_splits(s, d):
    ...
    if d<0:
        ...
        return sum([num_splits(s[:i]+s[i+1:],d-s[i]) for i in range(len(s))])
...
class CheckingAccount(Account):
    ...
    def deposit_check(self,Check):
        if self.holder==Check.master and not Check.deposited:
